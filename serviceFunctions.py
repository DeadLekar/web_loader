...
def get_insert_command(dict,table_name):
    cmd_insert_start = "INSERT INTO {}(".format(table_name)  # sql insert command text
    cmd_insert_end = ") VALUES ("
    for key in dict:
        cmd_insert_start += "{},".format(key)
        if is_digit(dict[key]):
            cmd_insert_end += "{},".format(dict[key])
        else:
            cmd_insert_end += "'{}',".format(dict[key])
    cmd_total = cmd_insert_start[:len(cmd_insert_start)-1] + cmd_insert_end[:len(cmd_insert_end)-1] + ')'
    return cmd_total
...
class WebSite:
    visited_links = []
    driver = None
    site_id = -1  # defined in children classes
    cat = ''
    cat_id = -1
    cards_html_classes = []
    next_page_class = ''

    def __init__(self, url, driver, cat, cat_id, cr_conn):
        self.driver = driver
    ...
    def get_cards_single(self):
        ...
        if not items:
            ...
            for item in items:
                self.cards.append(self.get_data(item, self.driver.current_url))
    ...
    def make_test(self):
        FIELDS_CNT = 9  # max fields' number in the card
        self.get_cards_single()
        len_sum = 0
        if self.cards:
            for c in self.cards:
...
class WebSite_Wildberries(WebSite):
    ...
    def get_data(self, soup, url):
        ...
        data_dict['webSiteID'] = self.site_id
        data_dict['categoryName'] = self.cat
        data_dict['categoryNameID'] = self.cat_id
        return data_dict
...
class WebSite_Lamoda(WebSite):
    ...
    next_page_class = 'paginator__next'
    def get_data(self, soup, url):
        # brand
        data_dict = {}
...
class WebSite_Kupivip(WebSite):
    ...
    next_page_class = 'icon-arrow right'
    def get_data(self, soup, url):
        # brand
        data_dict = {}
